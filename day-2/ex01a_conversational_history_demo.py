...
def main():
    ...
    while True:
        ...
        if user_input == "/q":
            break
        if user_input.startswith("/session_id"):        # /session_id:vinod
            curr_user = user_input.split(":")[1]

        resp = conversational_chain.invoke(
            {"user_message": user_input},       # prompt variable inputs
            config = {"configurable": { "session_id": curr_user }}
            )
        print(f"ai > {resp}")
        print(f"[DEBUG] session keys = {SESSION_STORE.keys()}")
